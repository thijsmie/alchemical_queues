...
def producer(
    index: int, queue_factory: Callable[[], AlchemicalQueues], num_samples: int
):
...
numbers = [(1, 1, 1000), (2, 2, 100), (2, 10, 100), (10, 2, 50)]


@pytest.mark.parametrize("num_producers,num_consumers,num_samples", numbers)
def test_stress(
    queue_factory: Callable[[], AlchemicalQueues],
    num_producers: int,
    num_consumers: int,
    num_samples: int,
):
    ...
    producers = [
        Thread(target=producer, args=(i, queue_factory, num_samples))
        for i in range(num_producers)
    ]
    cqueues: List[Queue] = [Queue() for i in range(num_consumers)]
    consumers = [
        Thread(target=consumer, args=(cqueues[i], queue_factory))
